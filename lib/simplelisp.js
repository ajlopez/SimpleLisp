...
function compile(text, context) {
    ...
    if (code)
        if (result)
            result = '(function () { ' + result + '; return ' + code + '; })()';
        else
            result = code;
...
function compileList(list, context) {
    ...
    if (symbol.isSymbol(head)) {
        ...
        if (name[0] == '.') {
            var result = compileExpression(list.rest().first(), context) + name + '(';
            var tail = list.rest().rest();
            var narg = 0;
        ...
        if (name[0] == ':') {
            var result = compileExpression(list.rest().first(), context) + '.' + name.substring(1);
