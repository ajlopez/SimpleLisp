
const symbol = require('./symbol');

function List(head, tail) {
    this.head = head;
    this.tail = tail;
};

List.prototype.first = function () {
    return this.head;
};

List.prototype.rest = function () {
    return this.tail;
};

List.prototype.asString = function () {
    let result = '(' + asString(this.head);

    for (let tail = this.tail; tail != null; tail = tail.tail)
        result += ' ' + asString(tail.head);

    return result + ')';
};

function EmptyList() {
};

EmptyList.prototype.first = function () {
    return null;
};

EmptyList.prototype.rest = function () {
    return null;
};

EmptyList.prototype.asString = function () {
    return '()';
};

const empty = new EmptyList();

function asString(obj) {
    if (obj === null)
        return 'nil';

    if (obj.asString)
        return obj.asString();

    return obj.toString();
}

function createList() {
    let result = null;

    for (let k = arguments.length; k--;)
        result = new List(arguments[k], result);

    if (result == null)
        return empty;

    return result;
};

function isList(obj) {
    return obj instanceof List || obj instanceof EmptyList;
}

module.exports = {
    createList: createList,
    newList: function (head, tail) { return new List(head, tail); },
    isList: isList
};

